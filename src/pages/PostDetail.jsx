...
const PostDetail = () => {
  ...
  const checkPostDetailInputAnswer=(e)=>{ //유효성 검사 닉네임
  ...
  const onClickNavigatePosts = ()=>{ //메인으로 가기
    navigate('/posts')
  }
      ...
      <StPostsBox>
        ...
        <StPostsDetailInfoBox>
          <StPostsDetailInfoWrite>
            <StPostsDetailInfoWriteAuth>출제자 : {postDetail?.nickname}</StPostsDetailInfoWriteAuth>
            <StPostsDetailInfoWriteDate>출제일 : {dateEdit}</StPostsDetailInfoWriteDate>
            <StPostsDetailInfoWriteDifficulty>난이도 : {star}</StPostsDetailInfoWriteDifficulty>
          </StPostsDetailInfoWrite>
          {
            ...
            ? (
                <StPostsDetailInfoAnswer>
                  ...
                  <StPostsDetailInfoAnswerForm onSubmit={onSubmitPostsDetailAnswer}>
                    <InputWithLabelDefault inputRef={answerRef} autoFocus="autofocus" inputType="text" inputId="postsDetailInputAnswer"
                    inputValue={valueAnswer} inputPaceholder="정답 입력하기"
                    width="90%" onChange={onChangeValueAnswer}  validMessage={validMessageAnswer}
                    onBlur={checkPostDetailInputAnswer} className="postDetailInputAnswer"
                    />
        ...
        <BsFillArrowLeftCircleFill onClick={onClickNavigatePosts}
        className="iconArrow prevIcon" aria-label="메인 페이지로 이동"/>
      </StPostsBox>
    </StPostsWrap>
  );
};
